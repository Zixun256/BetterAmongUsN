...
using System.Linq;
using static BetterAmongUsN.Translator;
...
internal class EAC
{
        ...
        try
        {
            ...
            switch (rpc)
            {
                ...
                case RpcCalls.CheckName:
                    if (!Modules.GameStates.IsLobby)
                    {
                        WarnHost();
                        Report(pc, "CheckName out of Lobby");
                        HandleCheat(pc, "CheckName out of Lobby");
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】CheckName out of lobby，已驳回", "EAC");
                        return true;
                    }
                    break;
                /*
                ...
                */
                ...
                case RpcCalls.SendChat:
                    ...
                    {
                        Report(pc, "非法消息");
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送非法消息，已驳回", "EAC");
                        return true;
                    }
                    break;
                case RpcCalls.StartMeeting:
                    ...
                    Logger.Fatal($"非法设置玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】的游戏名称，已驳回", "EAC");
                    return true;
                case RpcCalls.ReportDeadBody:
                    var bodyid = sr.ReadByte();
                    if (!Modules.GameStates.IsInGame)
                    {
                        if (!LobbyDeadBodies.Contains(bodyid))
                        {
                            ...
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】非游戏内开会，已驳回", "EAC");
                            return true;
                        }
                        else
                        {
                            Report(pc, "Try to Report body out of game B (May be false)");
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】尝试举报可能被非法击杀的尸体，已驳回", "EAC");
                        }
                    }
                    if (ReportTimes.TryGetValue(pc.PlayerId, out int rtimes))
                    {
                        if (rtimes > 14)
                        {
                            ...
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】刷报告尸体满14次，已驳回", "EAC");
                            return true;
                        }
                    }
                    break;
                case RpcCalls.CheckColor:
                    if (!Modules.GameStates.IsLobby)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】check color out of lobby，已驳回", "EAC");
                        return true;
                    }
                    break;
                ...
                case RpcCalls.SetColor:
                    ...
                    Logger.Fatal($"Directly SetColor【{pc.GetClientId()}:{pc.GetClient().PlayerName}】已驳回", "EAC");
                    return true;
                case RpcCalls.CheckMurder:
                    if (Modules.GameStates.IsLobby)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】非法检查击杀，已驳回", "EAC");
                        return true;
                    }
                    break;
                case RpcCalls.MurderPlayer:
                    ...
                    if (Modules.GameStates.IsLobby)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】大厅直接击杀，已驳回", "EAC");
                        return true;
                    }
                    Report(pc, "Directly Murder Player");
                    HandleCheat(pc, "Directly Murder Player");
                    Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】直接击杀，已驳回", "EAC");
                    return true;
                case RpcCalls.CheckShapeshift:
                    if (Modules.GameStates.IsLobby)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】大厅直接变形，已驳回", "EAC");
                        return true;
                    }
                    break;
                case RpcCalls.Shapeshift:
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】直接变形，已驳回", "EAC");
                        return true;
                    }
                ...
                case RpcCalls.StartAppear:
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName} {sreason}，已驳回", "EAC");
                        return true;
                    }
                case RpcCalls.SendChatNote:
                    ...
                    Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】直接Send ChatNote，已驳回", "EAC");
                    return true;
            }
            switch (callId)
            {
                case 101: // Aum Chat
                    try
                    {
                        ...
                        if (!flag)
                        {
                            ...
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送AUM聊天，已驳回", "EAC");
                            return true;
                        }
                    }
                    catch
                ...
                case unchecked((byte)42069): // 85 AUM
                    try
                    {
                        ...
                        if (aumid == pc.PlayerId)
                        {
                            ...
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送AUM RPC，已驳回", "EAC");
                            return true;
                        }
                    }
                    catch
                ...
                case 119: // KN Chat
                    try
                    {
                        ...
                        if (!flag)
                        {
                            ...
                            Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送KN聊天，已驳回", "EAC");
                            return true;
                        }
                    }
                    catch
                ...
                case 250: // KN
                    if (sr.BytesRemaining == 0)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送KN RPC，已驳回", "EAC");
                        return true;
                    }
                    break;
                case unchecked((byte)420): // 164 Sicko
                    if (sr.BytesRemaining == 0)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】发送Sicko RPC，已驳回", "EAC");
                        return true;
                    }
                    break;
                case 7:
                case 8:
                    if (!Modules.GameStates.IsLobby)
                    {
                        WarnHost();
                        Report(pc, "非法设置颜色");
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】非法设置颜色，已驳回", "EAC");
                        return true;
                    }
                    break;
                case 5:
                    ...
                    if (Modules.GameStates.IsInGame)
                    {
                        WarnHost();
                        Report(pc, "非法设置游戏名称");
                        Logger.Fatal($"非法修改玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】的游戏名称，已驳回", "EAC");
                        return true;
                    }
                    break;
                case 47:
                    if (Modules.GameStates.IsLobby)
                    {
                        WarnHost();
                        Report(pc, "非法击杀");
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】非法击杀，已驳回", "EAC");
                        return true;
                    }
                    break;
                case 38:
                    if (Modules.GameStates.IsInGame)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】游戏内改等级，已驳回", "EAC");
                        return true;
                    }
                ...
                case 43:
                    if (Modules.GameStates.IsInGame)
                    {
                        ...
                        Logger.Fatal($"玩家【{pc.GetClientId()}:{pc.GetClient().PlayerName}】游戏内改皮肤，已驳回", "EAC");
                        return true;
                    }
                    break;
            }
    ...
    {
        if (Modules.GameStates.IsLocalGame)
        {
            return false;
        }
        ...
        if (systemType == SystemTypes.Sabotage) //Normal sabotage using buttons
        {
            if (player.Data.RoleType != RoleTypes.Impostor)
            {
                ...
                Logger.Fatal($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】Bad Sabotage A，已驳回", "EAC");
                return true;
            }
        ...
        if (MeetingHud.Instance && MeetingHud.Instance.state != MeetingHud.VoteStates.Animating || Modules.GameStates.IsExilling)
        {
            WarnHost();
            Report(player, "Bad Sabotage D : In Meeting");
            Logger.Fatal($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】Bad Sabotage D，已驳回", "EAC");
            return true;
        }
    ...
    YesCheat:
        {
            ...
            Logger.Fatal($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】Bad Sabotage C，已驳回", "EAC");
            return true;
        }
    }
    ...
    public static bool RpcReportDeadBodyCheck(PlayerControl player, NetworkedPlayerInfo target)
    {
        ...
        if (!Modules.GameStates.IsInGame)
        {
            ...
            Logger.Fatal($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】非游戏内开会，已驳回", "EAC");
            return true;
        }
        ...
        if (ReportTimes[player.PlayerId] >= 14)
        {
            ...
            Logger.Fatal($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】刷报告尸体满14次，已驳回", "EAC");
            return true;
        }
        if (MeetingHud.Instance)
        {
            //Cancel rpc report body if a meeting is already held
            Logger.Info($"玩家【{player.GetClientId()}:{player.Data.PlayerName}】在会议期间开会，已驳回", "EAC");
            return true;
        }
    ...
    public static bool PlayerPhysicsRpcCheck(PlayerPhysics __instance, byte callId, MessageReader reader)
    {
        ...
        if (Modules.GameStates.IsLobby && rpcType is not RpcCalls.Pet and not RpcCalls.CancelPet)
        {
            WarnHost();
            ...
            Logger.Fatal($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to {rpcType} in lobby.", "EAC_physics");
            return true;
        }
        ...
        switch (rpcType)
        {
            ...
            case RpcCalls.ExitVent:
                ...
                if (!hasVent(ventid))
                {
                    if (AmongUsClient.Instance.AmHost)
                    {
                        ...
                        Logger.Fatal($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to enter a unexisting vent. {ventid}", "EAC_physics");
                    }
                    else
                    {
                        // Not sure whether host will send null vent to a player huh
                        Logger.Warn($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to enter a unexisting vent. {ventid}", "EAC_physics");
            ...
            case RpcCalls.BootFromVent:
                ...
                //        Logger.Fatal($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to boot from a unexisting vent. {ventid2}", "EAC_physics");
                //    }
                //    else
                //    {
                //        // Not sure whether host will send null vent to a player huh
                //        // Nah, host may send 99 boot from vent, which is stupid
                //        Logger.Warn($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to boot from a unexisting vent. {ventid2}", "EAC_physics");
                ...
                Logger.Fatal($"【{player.GetClientId()}:{player.Data.PlayerName}】 sent boot from vent, can be spoofed.", "EAC_physics");
                break;
            ...
            case RpcCalls.ClimbLadder:
                ...
                if (!hasLadder(ladderId))
                {
                    if (AmongUsClient.Instance.AmHost)
                    {
                        ...
                        Logger.Fatal($"【{player.GetClientId()}:{player.Data.PlayerName}】 attempted to climb a unexisting ladder.", "EAC_physics");
                    }
                    return true;
                }
                if (player.AmOwner)
    ...
    public static void Report(PlayerControl pc, string reason)
    {
        ...
        string msg = $"{pc.GetClientId()}|{pc.FriendCode}|{pc.Data.PlayerName}|{pc.Data.Puid}|{reason}";
        //Cloud.SendData(msg);
        Logger.Fatal($"EAC报告：{msg}", "EAC Cloud");
        Logger.SendInGame(string.Format(GetString("Message.NoticeByEAC"), $"{pc?.Data?.PlayerName} | {pc.Data.Puid}", reason));
    }
